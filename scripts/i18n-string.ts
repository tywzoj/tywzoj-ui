...
const enumFileName = "locale.strings.gen.ts";
const errorMapFileName = "error-map.gen.ts";
...
function main() {
    ...
    const errorMapContent = generateErrorMap(rawStrings);
    ...
    writeFeature(typeContent, enumContent, errorMapContent);
}
...
function generateEnums(localizedStrings: ILocalizedString[], idMap: IStringIdMap): string {
    ...
    for (const localizedString of localizedStrings) {
        ...
        * e.g. ${localizedString.value ?? "BAD STRING"}
        */\n`;
...
function generateErrorMap(localizedStrings: ILocalizedString[]): string {
    let content = "";
    content += '// This file is auto-generated by "yarn i18n"\n';
    content += "// Do NOT modify this file directly.\n\n";

    content += `import { default as CE_Strings } from "./${enumFileName}";\n\n`;

    content += "export const errorIdMap: Record<number, CE_Strings> = {\n";

    localizedStrings.forEach((localizedString) => {
        const name = localizedString.name;
        if (name.startsWith("ERROR_")) {
            const errorString = name.split("_")[1];
            const idError = Number.parseInt(errorString, 10);
            if (Number.isInteger(idError)) {
                content += `[${idError}]: CE_Strings.${name},\n`;
            }
        }
    });
...
function writeFeature(typeContent: string, enumContent: string, errorMapContent: string) {
    ...
    const typePath = pathResolve(featureDir, typeFileName);
    fs.writeFileSync(typePath, typeContent);
    const enumPath = pathResolve(featureDir, enumFileName);
    fs.writeFileSync(enumPath, enumContent);
    const errorIdMapPath = pathResolve(featureDir, errorMapFileName);
